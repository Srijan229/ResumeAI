  ...
  return (
    <section className="min-w-0 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
        ...
        <button disabled={busy} onClick={compile} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-neutral-950 px-4 py-2 text-sm font-medium text-white disabled:opacity-60">
          <FileText size={16} />
          Compile LaTeX to PDF
        </button>
        {pdfPath ? (
          <a href={pdfPath} download="tailored-resume.pdf" className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-neutral-300 px-4 py-2 text-sm">
            <Download size={16} />
            Download PDF
          </a>
        ) : null}
        <button disabled={busy} onClick={coverLetter} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md border border-neutral-300 px-4 py-2 text-sm disabled:opacity-60">
          <Mail size={16} />

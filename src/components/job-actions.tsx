  ...
  return (
    <section className="min-w-0 rounded-lg border border-neutral-200 bg-white p-5">
      ...
      <div className="flex flex-col gap-3">
        <button disabled={busy} onClick={analyze} className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-md bg-neutral-950 px-4 py-2 text-sm font-medium text-white disabled:opacity-60">
          <Brain size={16} />
          Analyze job
        </button>
        <select value={resumeId} onChange={(event) => setResumeId(event.target.value)} className="min-h-10 w-full min-w-0 rounded-md border border-neutral-300 px-3 text-sm">
          {resumes.map((resume) => (
        ...
        <button disabled={busy || !resumeId} onClick={tailor} className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-md border border-neutral-300 px-4 py-2 text-sm disabled:opacity-60">
          <Wand2 size={16} />
          Tailor resume
        </button>
      </div>
